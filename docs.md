...
**功能特性：**
- 从 Vault API 获取授权证书列表
- 支持选择性同步证书
- 支持 Standard 和 CloudFlare Origin 证书
- 定时自动同步（每小时）
- 证书权限自动管理

**持久化数据：**
- 路径：`/srv/server-toolkit/storage/cert-sync/`
- 内容：同步脚本、日志文件
- 证书存储：`/certs/[domain]/`
...
- 查看：`tail -f /srv/server-toolkit/storage/cert-sync/sync.log`
