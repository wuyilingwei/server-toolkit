...
# 定义期望的配置行
EXPECTED_LINE="$SWAPFILE none swap sw 0 0"

# 检查是否已存在完全一致的配置
if grep -Fxq "$EXPECTED_LINE" /etc/fstab; then
    echo "[+] fstab 中已存在正确的 Swap 配置，无需修改"
else
    echo "[*] 更新 fstab 配置..."
    ...
    # 移除所有关于此 SWAPFILE 的旧配置 (避免重复)
    # 使用 | 作为分隔符，防止路径中包含 / 导致 sed 报错
    sed -i "\|^[[:space:]]*$SWAPFILE[[:space:]]|d" /etc/fstab

    # 添加新配置
    echo "$EXPECTED_LINE" >> /etc/fstab
