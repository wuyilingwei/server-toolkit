#!/bin/bash
# Firewall Whitelist Worker Script - Persistent Security Synchronization
# (Formerly: SSH Security Worker)
...
# Check both new and legacy storage directories
if [ -f "$STORAGE_DIR/firewall_mode.conf" ]; then
    MODE_CONFIG="$STORAGE_DIR/firewall_mode.conf"
elif [ -f "$LEGACY_STORAGE_DIR/firewall_mode.conf" ]; then
    MODE_CONFIG="$LEGACY_STORAGE_DIR/firewall_mode.conf"
else
    MODE_CONFIG="$STORAGE_DIR/firewall_mode.conf"
fi
...
cleanup_drop() {
    # 清理所有规则（包括旧版本的标签）
    iptables -S INPUT | grep "$LEGACY_IPTABLES_TAG" | sed "s/-A/iptables -D/" | bash 2>/dev/null
    iptables -S INPUT | grep "$IPTABLES_TAG" | sed "s/-A/iptables -D/" | bash 2>/dev/null
}
...
if [ -n "$EXISTING_TYPE" ] && [ "$EXISTING_TYPE" != "hash:net" ]; then
    ...
    # Try both old and new tags
    iptables -D INPUT -m set --match-set "$IPSET_NAME" src -j ACCEPT -m comment --comment "$LEGACY_IPTABLES_TAG" 2>/dev/null
    iptables -D INPUT -m set --match-set "$IPSET_NAME" src -j ACCEPT -m comment --comment "$IPTABLES_TAG" 2>/dev/null
...
# Clean old rules (including legacy tags)
iptables -S INPUT | grep "$LEGACY_IPTABLES_TAG" | sed "s/-A/iptables -D/" | bash 2>/dev/null
iptables -S INPUT | grep "$IPTABLES_TAG" | sed "s/-A/iptables -D/" | bash 2>/dev/null

# Level 1: Top priority whitelist ACCEPT (always first)
iptables -I INPUT 1 -m set --match-set "$IPSET_NAME" src -j ACCEPT -m comment --comment "$IPTABLES_TAG"
...
if [ "$FIREWALL_MODE" = "strict" ]; then
    ...
    iptables -I INPUT 2 -p tcp --dport "$SSH_PORT" -j DROP -m comment --comment "$IPTABLES_TAG"
